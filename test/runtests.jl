...
function test_and_sample_model(
    ...
    chain_tempered = AbstractMCMC.bundle_samples(
        samples_tempered[.!(getproperty.(states_tempered, :is_swap))],
        MCMCTempering.maybe_wrap_model(model),
        typeof(sampler) <: AbstractMCMC.AbstractSampler ? sampler : sampler(model),
...
function compare_chains(
    ...
    if compare_ess
        ...
        if isbroken
            @test_broken all(ess_tempered .≥ (ess .* target_ess_ratio))
        else
            @test all(ess_tempered .≥ (ess .* target_ess_ratio))
        end
    end
end
...
@testset "MCMCTempering.jl" begin
    ...
    @testset "Turing.jl" begin
        ...
        @testset "AdvancedHMC.jl" begin
            ...
            compare_chains(chain_hmc, chain_tempered, atol=0.2, compare_std=false, compare_ess=true, target_ess_ratio=0.5, isbroken=false)
        end
        ...
        @testset "AdvancedMH.jl" begin
            ...
            compare_chains(chain_mh, chain_tempered, atol=0.4, compare_std=false, compare_ess=true, target_ess_ratio=0.5, isbroken=false)
        end
    end
end
